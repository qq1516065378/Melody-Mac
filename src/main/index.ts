...
// Disable hardware acceleration and sandbox for dev environment (sandbox compatibility)
...
import MacOSIntegration from "@main/macos-integration";
...
// Dev mode: use project-local data directory to avoid sandbox permission issues
if (process.defaultApp || process.env.NODE_ENV === "development") {
    try {
        const devDataPath = path.resolve(process.cwd(), ".dev-data");
        if (!fs.existsSync(devDataPath)) {
            fs.mkdirSync(devDataPath, { recursive: true });
        }
        app.setPath("appData", devDataPath);
        app.setPath("userData", path.resolve(devDataPath, "userData"));
    } catch (e) {
        console.warn("Failed to set dev data path:", e);
    }
}
