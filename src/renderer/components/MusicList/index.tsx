...
const columnDef: ColumnDef<IMusic.IMusicItem>[] = [
    ...
    columnHelper.accessor("duration", {
        ...
        maxSize: 150,
        minSize: 48,
        cell: (info) => <MusicDuration musicItem={info.row.original} />,
        // @ts-ignore
        fr: 1,
    }),
...
function MusicMetadataLink(props: {
    type: "artist" | "album";
    musicItem: IMusic.IMusicItem;
    label?: string;
}) {
    const { type, musicItem, label } = props;
    const navigate = useNavigate();

    if (!label) return <span>-</span>;

    const openMetadata = async (event: React.MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
        const raw = musicItem as any;
        const embedded = type === "artist" ? raw.artistItem : raw.albumItem;
        const embeddedId = type === "artist" ? raw.artistId : raw.albumId;

        if (embedded?.id || embeddedId) {
            const item = embedded ?? {
                id: embeddedId,
                platform: musicItem.platform,
                ...(type === "artist"
                    ? { name: label, avatar: musicItem.artwork }
                    : { title: label, artwork: musicItem.artwork, artist: musicItem.artist }),
            };
            navigate(
                `/main/${type}/${encodeURIComponent(item.platform ?? musicItem.platform)}/${encodeURIComponent(item.id)}`,
                { state: { [type === "artist" ? "artistItem" : "albumItem"]: item } },
            );
            return;
        }

        const plugin = PluginManager.getPluginByPlatform(musicItem.platform);
        if (!plugin || (plugin.supportedSearchType && !plugin.supportedSearchType.includes(type))) {
            toast.info(`当前音源不支持${type === "artist" ? "歌手" : "专辑"}跳转`);
            return;
        }
        ...
        try {
            const result = await PluginManager.callPluginDelegateMethod(
                ...
                1,
                type,
            ) as IPlugin.ISearchResult<typeof type>;
            const normalizedLabel = label.replace(/\s+/g, "").toLocaleLowerCase();
            const item = result?.data?.find((candidate: any) =>
                String(type === "artist" ? candidate.name : candidate.title)
                    .replace(/\s+/g, "")
            ...
            navigate(
                `/main/${type}/${encodeURIComponent(item.platform)}/${encodeURIComponent(item.id)}`,
                { state: { [type === "artist" ? "artistItem" : "albumItem"]: item } },
            );
        } catch {
            toast.info(`没有找到“${label}”的${type === "artist" ? "歌手" : "专辑"}页面`);
        }
    };
    ...
    return (
        <button
            type="button"
            className="music-metadata-link"
            title={`查看${type === "artist" ? "歌手" : "专辑"}：${label}`}
...
// 3rem row height plus the 4px vertical table spacing in tables.scss.
// Keeping virtualization in sync prevents rows from visually colliding.
const estimizeItemHeight = 43;
